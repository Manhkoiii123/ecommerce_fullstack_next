...
CREATE TABLE `Store` (
    ...
    `status` ENUM('PENDING', 'BANNED', 'DISABLED', 'ACTIVE') NOT NULL DEFAULT 'PENDING',
    `averageRating` DOUBLE NOT NULL DEFAULT 0,
    `featured` BOOLEAN NOT NULL DEFAULT false,
    `numReviews` INTEGER NOT NULL DEFAULT 0,
    `returnPolicy` VARCHAR(191) NOT NULL DEFAULT 'Return in 30days.',
...
CREATE TABLE `FlashSale` (
    ...
    INDEX `FlashSale_storeId_idx`(`storeId`),
...
CREATE TABLE `Notification` (
    `id` VARCHAR(191) NOT NULL,
    `type` ENUM('NEW_ORDER', 'ORDER_STATUS_CHANGE', 'PAYMENT_RECEIVED', 'PAYMENT_FAILED', 'ORDER_SHIPPED', 'ORDER_DELIVERED', 'ORDER_CANCELLED', 'LOW_STOCK', 'REVIEW_RECEIVED', 'SYSTEM_UPDATE') NOT NULL,
